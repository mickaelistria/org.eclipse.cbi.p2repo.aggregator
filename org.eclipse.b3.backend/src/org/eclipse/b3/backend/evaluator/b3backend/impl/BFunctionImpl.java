...
public class BFunctionImpl extends BExpressionImpl implements BFunction {
	...
	public Type getReturnType() {
		...
		if(returnType != null && returnType instanceof EObject && ((EObject) returnType).eIsProxy()) {
			...
			if(returnType != oldReturnType) {
				if(eNotificationRequired())
					eNotify(new ENotificationImpl(this, Notification.RESOLVE, B3backendPackage.BFUNCTION__RETURN_TYPE,
		...
		if(functionParameterTypes.length > 0) { // if function takes no parameters, there is no binding to be done
			...
			for(int i = 0; i < limit; i++) {
				...
				if(!(TypeUtils.isAssignableFrom(functionParameterTypes[i], t)))
					throw new B3IncompatibleTypeException(functionParameterNames[i], functionParameterTypes[i],
				...
				if(o != null) {
					...
					if(!TypeUtils.isAssignableFrom(functionParameterTypes[limit], t))
						throw new B3IncompatibleTypeException(functionParameterNames[limit],
								functionParameterTypes[limit], parameters[limit].getClass());
				}
				// ok
	...
	public void setReturnType(Type newReturnType) {
		if(newReturnType != returnType) {
			...
			eNotify(new ENotificationImpl(this, Notification.SET, B3backendPackage.BFUNCTION__RETURN_TYPE,
