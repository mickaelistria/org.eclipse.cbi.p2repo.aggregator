...
public class MetadataMirrorGenerator extends BuilderPhase {
	...
	public void run(IProgressMonitor monitor) throws CoreException {
		...
		try {
			...
			IMetadataRepository targetMdr = builder.getAggregationMetadataRepository();
			...
			for(Contribution contrib : contribs) {
				...
				for(MappedRepository repo : repos) {
					...
					if(iusToMirror != null) {
						String msg = format(
							"Mirroring meta-data consisting of %d units from %s", iusToMirror.size(), mdr.getLocation());
						LogUtils.info(msg);
